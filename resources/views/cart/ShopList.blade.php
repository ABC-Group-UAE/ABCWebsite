...
<head>
    ...
    <meta name="description" content="Favourite products from the collections">
  ...
  <link rel="canonical" href="https://abcgroupuae.com/en/CartView">
    ...
    <div class="form-wrapper w-clearfix"><a href="{{route('Cart.View',app()->getlocale())}}" aria-current="page" class="close w--current">x </a>
      ...
      <div class="w-form">
        <form id="email-form-4" name="email-form-4" data-name="Email Form 4" method="post" id="ContactForm" action="{{ route('shoplist.enquire',app()->getlocale()) }}">
          ...
          <input type="tel" class="w-input" autofocus="true" maxlength="256" name="PhNo" data-name="PhNo" placeholder="971505047899" id="PhNo" required="" pattern="971[0-9]{9}" oninvalid="setCustomValidity('Please enter a valid phone number in  971XXXXXXXXX format')"  onchange="try{setCustomValidity('')}catch(e){}">
  ...
  <div class="main-container w-container" id ="ShopList">
    <div id ="CartContent">
          ...
          ?>
              ...
              @php 
			 if(\Cart::session($SId)->isEmpty()) { 
             echo "style='display:flex;flex-direction:column;align-items:center;justify-content:center;height:60%;padding:100px;padding-bottom:5px;'"; } else { echo "style='display:none;'";}
			@endphp  id="emptycart" > 
                <img src="/abccontentbucket.s3.me-south-1.amazonaws.com/images/empty-state-cart.png"  class="empty-cart"/>
                                            ...
                                            <div id="w-node-772a9433cfff-fa8c5f7d" class="procontentdiv">
                                              <div class="div-block-90">
                                                <a href="{{route('Product.description',[app()->getlocale(),$product->Product_id])}}" class="link-block-2 w-inline-block">
                                                  <div class="div-block-75"><img src="/abccontentbucket.s3.me-south-1.amazonaws.com/products/{{$product->MainImage}}" loading="lazy" alt="" class="image-51" id = "{{'Productpic_'.$row}}"></div>
                                                </a>
                                              </div>
                                            </div>
                                            <a href="{{route('Product.description',[app()->getlocale(),$product->Product_id])}}" class="link-block-2 w-inline-block">
                      ...
                      <div class="rowid" id="{{'Wishrow'.$row}}">
                        <div class="wishlitrow-grid-without-price">
                          <div class="procontentdiv">
                            <div class="div-block-90">
                              <a href="{{route('Product.description',[app()->getlocale(),$product->Product_id])}}" class="link-block-2 w-inline-block">
                                <div class="div-block-75 combo"><img src="/abccontentbucket.s3.me-south-1.amazonaws.com/products/{{$product->MainImage }}" loading="lazy" alt="" class="image-52" id = "{{'WishProductpic_'.$row}}"></div>
                              </a>
                            </div>
                          </div>
                          <a href="{{route('Product.description',[app()->getlocale(),$product->Product_id])}}" class="link-block-2 w-inline-block">
      ...
      function RemoveFromCart(rowid,e){
           ...
           // e.stopP
            ...
            if($("#similar"+productId).length){
                              var imageid = "similar"+productId+"cartimg";
                              var textid = "similar"+productId+"carttext";
                              var imageurl = "/abccontentbucket.s3.me-south-1.amazonaws.com/images/Shoplist-Ikea-rzk-version_1.svg";
                              console.log('imageurl = '+imageurl);
                              document.getElementById(imageid).src = imageurl;
                              $("#"+textid).empty();
                              $("#"+textid).append("Add to Cart");
            }
                  ...
                  success:function(result,e) {
                   // alert(result.message+"and"+ result.count+"and"+ result.success );
                    ...
                    if(result.count > 0){
                      ...
                      $("#emptycart").css('flex-direction','column');
                      $("#emptycart").css('align-items','center');
