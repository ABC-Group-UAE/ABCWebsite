...
class WishListController extends Controller
{
    ...
    public function addToWhishList()
	{
		...
		if(!$AlreadyAdded){
            $item= $wish_list->add($id, $product->product_name, $product->Unit_price, $qty, $customAttributes);
            
        }
        $addedItem = $wish_list->get($id);
        $count =$wish_list->getContent()->count();
        $summedPrice = $wish_list->get($id)->getPriceSum();
    ...
    function wishListUpdate(){
       if(Session::has('id')){
        ...
        $addedItem = $wish_list->get($id);
        $summedPrice =0;
        $summedPrice = app('wishlist')->get($id)->getPriceSum();
        return response(array(
            'success' => true,
             'data' => $item,
             'cartitem' => $addedItem,
